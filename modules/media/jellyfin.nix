...
#
# Jellyfin uses the official image with /dev/dri passthrough for QSV.
# Runs as media:media via `user`. Jellyseerr wires to Jellyfin + arr.
# ================================================================
{ config, lib, pkgs, ... }:
let
...
in
{
  virtualisation.oci-containers.containers = {
    jellyfin = ml.onNet {
      ...
      # Intel Quick Sync: pass the render device. Official image runs as `user`.
      extraOptions = [
        "--user=${uidgid}"
